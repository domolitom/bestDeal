...
#
# HOWTO: Copy to .claude/hooks/block-orchestrator-tools.sh, chmod +x.
# Adjust the allow-list paths (search for "Allow") to match meta-files
# your orchestrator legitimately edits on this project.
#
...
if [[ "$TOOL_NAME" == "Edit" ]] || [[ "$TOOL_NAME" == "Write" ]]; then
  ...
  # Allow meta-files: CLAUDE.md, plans, memory, beads, agents, hooks, CI configs
  ...
  # Allow edits in worktrees (supervisors work here)
  ...
  # On feature branch — allow quick fixes with user approval
...
# Block --no-verify on commits (prevents skipping commit hooks)
