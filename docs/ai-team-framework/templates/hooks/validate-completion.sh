#!/bin/bash
#
# SubagentStop: Verify supervisor work is complete (no uncommitted changes left).
#
# HOWTO: Copy to .claude/hooks/validate-completion.sh, chmod +x.
# Adjust the completion phrase (BEAD.*COMPLETE) and the worktree path
# (.worktrees/bd-{ID}) if your conventions differ.
#
...
# Only validate explicit completions
[[ "$HAS_BEAD_COMPLETE" -lt 1 ]] && echo '{"decision":"approve"}' && exit 0

# Extract BEAD_ID from the completion message
...
# If no worktree (read-only supervisors like scout/reviewer), approve
