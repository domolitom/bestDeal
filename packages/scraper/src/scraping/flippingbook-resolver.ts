...
async function resolveViaFlippingBook(
  ...
  console.log(`[flippingbook] fetching ${baseUrl}`);
  ...
  console.log(
    `[flippingbook] got ${pages.length} pages for ${catalogId}`
  );
...
const flippingbookResolver: CatalogResolver = {
  name: "flippingbook",
  needsLastPage: false,
  resolve: resolveViaFlippingBook,
};

registerResolver(flippingbookResolver);
