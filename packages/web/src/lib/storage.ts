...
/**
 * Read-only filesystem storage adapter for the web app.
 */
class WebStorageAdapter implements StorageAdapter {
  ...
  async getCatalog(id: string): Promise<Catalog | null> {
    ...
    const pages: CatalogPage[] = pageFiles.map((f, i) => {
      ...
      return {
        number: num,
        imageUrl: `/data/catalogs/${parsed.country}/${parsed.store}/${id}/pages/${f}`,
        filename: f,
      };
    });
  ...
  async writeCatalogMeta(): Promise<void> {
    throw new Error("Web app storage is read-only");
  }
...
// Singleton instance
export const storage = new WebStorageAdapter(DATA_DIR);
